...
grep -q 'UPDATE_NOTICE_MS = 7500' "$SITE_DIR/assets/release-polish-v29.js"
...
grep -q "HOTFIX_REVISION = '20260809-static-supertonic-orientation-ui-v29-2'" "$SITE_DIR/service-worker.js"
...
if [[ "$REQUIRE_STATIC_SUPERTONIC" == "1" ]]; then
  ...
  if [[ "$expected_count" != 175 ]]; then
    echo "Der statische Sprachkatalog muss in v29 exakt 175 Supertonic-F1-Sätze enthalten: $expected_count" >&2
    exit 1
  fi
  ...
  grep -q '"staticSpeechCount": 175' "$audio_dir/build-summary.json"
  grep -q '"count": 175' "$audio_dir/build-summary.json"
  ...
  grep -q '"staticSpeechCount": 175' "$SITE_DIR/assets/guide-audio-catalog.json"
...
if [[ "$REQUIRE_STATIC_SUPERTONIC" == "1" ]]; then
  echo "DokoHilf $BUILD_ID als v29 mit 175 statischen Supertonic-F1-WAVs, verschachtelter Orientierungshilfe, kurzem Hey-Sprachstart und dezenter Versionsanzeige gebaut."
else
  echo "DokoHilf $BUILD_ID als v29-QA-Site gebaut; der vollständige Releasebuild verlangt separat exakt 175 statische Supertonic-F1-WAVs."
fi
