...
const context=await browser.newContext({viewport:{width:WIDTH,height:HEIGHT},deviceScaleFactor:SCALE,isMobile:true,hasTouch:true,serviceWorkers:'block',userAgent:PROFILE==='android'?'Mozilla/5.0 (Linux; Android 15; Pixel 9) AppleWebKit/537.36 Chrome/139 Mobile Safari/537.36':'Mozilla/5.0 (iPhone; CPU iPhone OS 18_7 like Mac OS X) AppleWebKit/605.1.15 Version/18.0 Mobile/15E148 Safari/604.1'});
...
await page.addInitScript(({profile})=>{
  ...
  const localCalls=[];const systemCalls=[];window.__DOKOHILF_LOCAL_VOICE_TEST_CALLS__=localCalls;window.__DOKOHILF_SYSTEM_SPEECH_TEST_CALLS__=systemCalls;
  window.__DOKOHILF_LOCAL_VOICE_TEST_ADAPTER__={async prepare(){return{backend:profile==='android'?'webgpu-test':'wasm-test',async synthesize(text){localCalls.push(String(text||''));const s=8000,n=640,d=n*2,b=new ArrayBuffer(44+d),v=new DataView(b),a=(o,x)=>[...x].forEach((c,i)=>v.setUint8(o+i,c.charCodeAt(0)));a(0,'RIFF');v.setUint32(4,36+d,true);a(8,'WAVE');a(12,'fmt ');v.setUint32(16,16,true);v.setUint16(20,1,true);v.setUint16(22,1,true);v.setUint32(24,s,true);v.setUint32(28,s*2,true);v.setUint16(32,2,true);v.setUint16(34,16,true);a(36,'data');v.setUint32(40,d,true);return{wav:b,latencyMs:6};}};}};
...
try{
  ...
  const voiceText=await page.locator('#voiceFocusText').innerText();assert(!/erledigt|tun nicht so|markiere/i.test(voiceText),'Voice zeigt interne Zustände.');assert(await voiceHelp.locator('[data-detail-help-value]').count()===4,'Vier Voice-Optionen fehlen.');assert(await page.evaluate(()=>window.__DOKOHILF_LOCAL_VOICE_TEST_CALLS__.length)===0,'Bestätigte Folgeantwort fiel in lokale Inferenz.');
