...
function setCached(text: string, wav: Uint8Array, model: string): void {
  const key = cacheKey(text);
...
function voicePrompt(text: string): string {
  return [
    'Sprich diesen deutschen Text zügig, natürlich und ruhig wie eine erfahrene Kollegin direkt neben der hörenden Person.',
    'Keine Moderation, kein Ansagerhythmus, keine überdeutliche Aussprache. Lies ausschließlich den Text:',
    text,
  ].join('\n');
}
...
function audioResponse(
  ...
  return new Response(wav, {
    ...
    headers: {
      ...
      'X-DokoHilf-TTS-Cache': cacheStatus,
    },
  });
}
...
Deno.serve(async (req: Request) => {
  ...
  if (cached) {
    return audioResponse(origin, cached.wav, cached.model, 'memory-cache', Date.now() - startedAt, 'HIT');
  }
