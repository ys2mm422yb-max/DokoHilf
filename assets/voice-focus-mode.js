(() => {
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .voice-focus-toolbar{position:absolute;top:4px;right:4px;z-index:5}
      .voice-focus-toolbar button{min-height:40px;padding:0 13px;border:1px solid rgba(11,107,82,.18);border-radius:999px;background:rgba(255,255,255,.88);color:#0b5d49;font-size:12px;font-weight:800;box-shadow:0 8px 24px rgba(8,67,50,.10);backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px)}
      .voice-focus-instruction{position:relative;z-index:2;width:min(720px,100%);margin:0 auto;padding:19px 20px;border:1px solid rgba(11,107,82,.16);border-radius:24px;background:rgba(255,255,255,.92);box-shadow:0 18px 46px rgba(8,67,50,.11);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px);text-align:center}
      ...
      .voice-focus-stage::before,.voice-focus-stage::after{content:'';position:absolute;z-index:-1;border-radius:50%;filter:blur(2px);pointer-events:none}
      .voice-focus-stage::before{width:330px;height:330px;left:50%;top:52%;transform:translate(-50%,-50%);background:radial-gradient(circle,rgba(21,154,121,.13),rgba(21,154,121,0) 70%)}
      .voice-focus-stage::after{width:210px;height:210px;right:3%;top:12%;background:radial-gradient(circle,rgba(96,68,173,.10),rgba(96,68,173,0) 72%)}
      .app-shell[data-mode="voice"] .main-content{width:100%!important;height:calc(100dvh - 70px);padding:0 14px calc(98px + env(safe-area-inset-bottom))!important;overflow:hidden}
      ...
      .app-shell[data-mode="voice"] .voice-focus-stage{display:flex!important;height:100%;max-width:780px;margin:0 auto;padding:18px 4px 8px;flex-direction:column;align-items:center;justify-content:center;gap:19px}
      ...
      @media(max-width:640px){
        ...
        .app-shell[data-mode="voice"] .voice-focus-stage{padding-top:10px;gap:13px;justify-content:center}
        .voice-focus-toolbar{top:1px;right:1px}
        .voice-focus-toolbar button{min-height:36px;padding:0 11px;font-size:11px}
        .voice-focus-instruction{padding:15px 15px;border-radius:20px}
      ...
      @media(max-height:690px){
        ...
        .voice-focus-instruction{padding:12px 14px}
        .voice-focus-instruction p{font-size:18px;margin-top:5px}
        ...
        .app-shell[data-mode="voice"] .voice-focus-stage .pause-button{margin-top:8px!important}
      }
  ...
  function ensureStage() {
    ...
    stage.innerHTML = `
      <div class="voice-focus-toolbar">
        <button type="button" data-switch-mode="chat" aria-label="Chatverlauf anzeigen">Chatverlauf</button>
      </div>
  ...
  function updateInstruction() {
    ...
    kicker.textContent = currentProgressLabel();
  }
  ...
  function observeChanges() {
    ...
    const workspace = document.getElementById('workspace');
    if (workspace && workspace.dataset.voiceFocusProgressObserver !== 'active') {
      workspace.dataset.voiceFocusProgressObserver = 'active';
      new MutationObserver(updateInstruction).observe(workspace, { childList: true, subtree: true, characterData: true });
    }
  }
