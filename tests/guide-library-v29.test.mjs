...
test('Berichtssuche bleibt Draft und kommt später', () => {
  assert.match(library, /<strong>Berichtssuche<\/strong><small>Wird fachlich noch überarbeitet · kommt später<\/small>/); assert.doesNotMatch(library, /LIBRARY_ORDER = \[[^\]]*berichtssuche/); assert.match(migration, /set status = 'draft'/);
});
...
test('Hauptmenü nutzt lokale Häufigkeit, individuelle Icons und 17 Top-Level-Anleitungen', () => {
  ...
  for (const icon of ['doctor', 'pulse', 'reportEdit', 'emergency', 'form', 'handover']) assert.match(library, new RegExp(`${icon}: '<path`));
  const order = library.match(/const LIBRARY_ORDER = \[([\s\S]*?)\];/); assert.ok(order); const slugs = [...order[1].matchAll(/'([^']+)'/g)].map(match => match[1]); assert.equal(slugs.length, 17); assert.equal(new Set(slugs).size, 17);
});
...
test('Mobile Renderfreigabe prüft sichtbaren Bibliothekszustand', () => {
  assert.match(mobileRender, /window\.__DOKOHILF_GUIDE_LIBRARY_V29__ === true/); assert.match(mobileRender, /label === 'Häufig genutzt'/); assert.match(mobileRender, /\.v29-frequent-guide/); assert.match(mobileRender, /legacyHidden/); assert.match(mobileRender, /Alle Anleitungen/); assert.match(mobileRender, /Berichtssuche/);
});
