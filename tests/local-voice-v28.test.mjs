...
test('Systemstimme bleibt blockiert', () => {
...
test('v28 Build-ID, Load-Order und Supertonic-PWA-Revision sind explizit', () => {
  ...
  assert.match(worker, /HOTFIX_REVISION = '20260807-static-supertonic-guides-v28-3'/);
  assert.match(worker, /LOCAL_VOICE_MODEL_CACHE/);
  assert.match(worker, /STATIC_AUDIO_CACHE/);
  assert.match(applyDetail, /20260807-static-supertonic-guides-v28-3/);
  assert.match(build, /static-supertonic-guide-v28/);
});

test('Modellgewichte werden nicht ins Repository eingebettet; nur erzeugte allgemeine Guide-WAVs gehen in Pages', () => {
