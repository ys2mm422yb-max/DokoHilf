...
test('GitHub-Build leitet die Gesamtzahl aus allen kontrollierten Sprachkatalogen ab', () => {
  ...
  assert.match(builder, /validate_published_entries/);
  assert.match(builder, /FORBIDDEN_PUBLISHED_FRAGMENTS/);
  assert.match(builder, /static_speech_count = len\(entries\)/);
  assert.doesNotMatch(builder, /STATIC_SPEECH_COUNT\s*=/);

  assert.equal(catalogs.base.entries.length, 134);
  ...
  assert.match(sourceCatalogText, /41 approved dokohilf_guides \/ 133 unique approved step texts plus greeting/);
  assert.doesNotMatch(sourceCatalogText, /Doku erweitert|Öffne oben den Reiter „Aufgaben“|Wähle darunter „Aktuelles“|Abwesend – Krankenhaus|das passende externe Angebot/);
  ...
  assert.match(sourceCatalogText, /Wähle den passenden Status aus\./);
